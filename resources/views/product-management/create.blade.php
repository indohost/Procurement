    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
            <div class="widget-content widget-content-area br-8">
                ...
                <form method="POST" action="{{ route('product-management.store') }}" class="row g-3 needs-validation"
                        ...
                        <select name="unit_stock" class="form-control" id="unit_stock"
                            ...
                            @forelse ($unitProducts as $unitProduct)
                                <option value="{{ $unitProduct }}"
                                    {{ $unitProduct === old('unit_stock') ? 'selected' : '' }}>
                                    {{ $unitProduct }}
                                </option>
                            @empty
                        ...
                        <select name="category" class="form-control" id="category"
                            ...
                            @forelse ($categoryProducts as $categoryProduct)
                                <option value="{{ $categoryProduct }}"
                                    {{ $categoryProduct === old('category') ? 'selected' : '' }}>
                                    {{ $categoryProduct }}
                                </option>
                            @empty
                                <option>Tidak Ada Data</option>
